...
def get_hr_prospects(pitcher_name, statcast_df, batter_df, pitcher_df, min_pitches=5, team=None, manual_name_mapping=None):
    ...
    if pitcher_statcast.empty:
        ...
        return pd.DataFrame(columns=['Name', 'Team', 'PA', 'hr_probability', 'matchup_score', 'Barrel%', 'HardHit%', 'wRC+', 'AVG', 'Flyball%', 'Pull%', 'K%', 'BB%', 'HR/FB', 'Pitcher_HR/9', 'Recent_Flyball%', 'Vulnerable_Pitch_Types', 'Batter_Hand'])
    ...
    # Calculate recent Flyball% (past week: May 6 to May 13, 2025)
    recent_statcast = statcast_df[
        (statcast_df['game_date'] >= '2025-05-06') & 
        (statcast_df['game_date'] <= '2025-05-13')
    ].copy()
    ...
    recent_flyball = recent_flyball[recent_flyball['events'] >= 5]  # Minimum events for reliability
    ...
    if not pitcher_metrics.empty:
        ...
        pitcher_hr9 = pitcher_df['HR/9'].mean()  # League average if pitcher data missing
    ...
    prospects = batter_summary.sort_values('matchup_score', ascending=False)[
        ['batter_name', 'Team', 'PA', 'weighted_hr_prob', 'matchup_score', 'Barrel%', 'HardHit%', 'wRC+', 'AVG', 'Flyball%', 'Pull%', 'K%', 'BB%', 'HR/FB', 'Pitcher_HR/9', 'Recent_Flyball%', 'pitch_type', 'batter_hand']
    ].rename(columns={
        'batter_name': 'Name',
